...
async def process_youtube(payload: YouTubeRequest):
    print(f"Crunching YouTube URL: {payload.url}")
    transcript = get_youtube_transcript(payload.url)

    if transcript.startswith("Error"):
        return {"status": "error", "message": transcript}

    ai_results = generate_study_materials(transcript, payload.options)
    if "error" in ai_results:
        return {"status": "error", "message": ai_results["error"]}

    return {"status": "success", "message": "Content crunched successfully!", "data": ai_results}
...
async def process_pdf(file: UploadFile = File(...), options: str = Form(...)):
    ...
    try:
        ...
        parsed_options = ["summary"]

    file_bytes = await file.read()
    pdf_text = extract_text_from_pdf(file_bytes)
    
    if pdf_text.startswith("Error") or not pdf_text.strip():
        return {"status": "error", "message": "Could not extract text from the document."}
        
    ai_results = generate_study_materials(pdf_text, parsed_options)
    if "error" in ai_results:
        return {"status": "error", "message": ai_results["error"]}

    return {"status": "success", "message": "PDF crunched successfully!", "data": ai_results}
...
async def process_text(payload: TextRequest):
    ...
    ai_results = generate_study_materials(payload.text, payload.options)
    if "error" in ai_results:
        return {"status": "error", "message": ai_results["error"]}

    return {"status": "success", "message": "Text crunched successfully!", "data": ai_results}
